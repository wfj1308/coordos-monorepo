...
-- Backfill contracts.project_ref <-> project_nodes.contract_ref safely.
-- Safety rules:
-- 1) Only fill empty fields, never overwrite existing non-empty bindings.
-- 2) Only use unique mapping candidates to avoid ambiguous joins.
-- 3) Scope all operations by tenant_id.
...
SELECT
  ...
  (SELECT COUNT(*) FROM contracts c WHERE c.tenant_id = :tenant_id::int AND c.deleted = FALSE) AS contracts_total,
  (SELECT COUNT(*) FROM contracts c WHERE c.tenant_id = :tenant_id::int AND c.deleted = FALSE AND COALESCE(c.project_ref, '') <> '') AS contracts_bound,
  (SELECT COUNT(*) FROM project_nodes pn WHERE pn.tenant_id = :tenant_id::int) AS project_nodes_total,
  (SELECT COUNT(*) FROM project_nodes pn WHERE pn.tenant_id = :tenant_id::int AND COALESCE(pn.contract_ref, '') <> '') AS project_nodes_bound;

-- Step A: project_nodes.contract_ref -> contracts.project_ref (parse v://{tenant}/contract/{id})
WITH parsed_refs AS (
  SELECT
...
unique_candidates AS (
  SELECT
    tenant_id,
    contract_id,
    MIN(project_ref) AS project_ref
...
-- Step B: project_nodes.legacy_contract_id -> contracts.legacy_id
...
-- Step C: contracts.project_ref -> project_nodes.contract_ref
WITH candidates AS (
  SELECT
...
-- Step D: contracts.legacy_id -> project_nodes.legacy_contract_id
...
-- Post-check: binding status + mismatch count.
SELECT
