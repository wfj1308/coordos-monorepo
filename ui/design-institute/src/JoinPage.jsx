...
export default function JoinPage() {
    ...
    try {
      ...
      if (Array.isArray(data.qualifications) && data.qualifications.length > 0) {
        const next = data.qualifications.map((item) => {
          const preset = QUAL_PRESETS.find(
            (q) => q.resourceType === item.resource_key || q.resourceType === item.qual_type,
          );
          return {
    ...
    try {
      if (engineers.length > 0) {
        ...
        engineers.forEach((row) => {
          lines.push(
            [row.name, row.idCard, row.registerType, row.specialty, row.certNo]
  ...
  const stepItems = [
    { id: 1, title: "步骤一", hint: "组织主体与命名空间" },
    { id: 2, title: "步骤二", hint: "资质范围与 Genesis" },
    { id: 3, title: "步骤三", hint: "执行体与工程师导入" },
    { id: 4, title: "步骤四", hint: "激活验证与 Resolver" },
  ];
  const currentStepMeta = stepItems.find((item) => item.id === step) || stepItems[0];
  const doneCount = stepItems.filter((item) => stepDone[item.id]).length;

  return (
    <main className="join-page di-page-shell">
      <div className="join-bg-orb join-bg-orb-a" />
      <div className="join-bg-orb join-bg-orb-b" />

      <div className="join-wrap">
        <SystemSectionHeader
          kicker="CoordOS / Join Wizard"
          title="设计院入网注册"
          subtitle="四步完成组织入网、资质注册、执行体导入与激活上线。"
          actions={[
        ...
        <section className="panel join-hero">
          ...
          <p className="join-subtitle">
            四步完成入网: 组织信息 → 资质注册 → 执行体导入 → 激活上线。资质将注册为 Genesis
            UTXO，人员将注册为可寻址执行体。
          </p>
          <div className="join-badges">
            {stepItems.map((item) => (
              <button
                key={item.id}
                type="button"
                className={`join-badge ${step === item.id ? "is-active" : ""} ${stepDone[item.id] ? "is-done" : "is-todo"}`}
        ...
        <section className="join-workspace">
          <aside className="panel join-step-sidebar">
            <div className="join-step-sidebar-head">
              <h2>入网步骤</h2>
              <p>按顺序推进，可随时回到已完成步骤复核。</p>
            </div>
            <div className="join-step-list">
              {stepItems.map((item) => (
                <button
                  ...
                  type="button"
                  onClick={() => setStep(item.id)}
                  className={`join-step-item ${step === item.id ? "is-active" : ""} ${stepDone[item.id] ? "is-done" : ""}`}
                >
                  <div className="join-step-index">0{item.id}</div>
                  <div>
                    <div className="join-step-title">{item.title}</div>
                    <div className="join-step-hint">{item.hint}</div>
                  </div>
                </button>
              ))}
            </div>
          ...
          <div className="join-content-stack">
            ...
            <section className="panel join-stage-banner">
              <div>
                <p className="join-stage-kicker">当前步骤</p>
                <h3>{currentStepMeta.title}</h3>
                <p>{currentStepMeta.hint}</p>
              </div>
              <div className="join-stage-stats">
                <span>{`已完成 ${doneCount}/4`}</span>
                <span>{`当前序号 0${step}`}</span>
              </div>
            </section>
              ...
              <section className="panel join-card">
                ...
                <label className="join-field">
                  ...
                  <input
                    ...
                    onChange={(e) => callExtract(e.target.files?.[0])}
                  />
                </label>
                ...
                <div className="join-grid">
                  <label className="join-field">
                    ...
                    <input
                      value={org.shortCode}
                      onChange={(e) => updateOrg("shortCode", e.target.value)}
                      placeholder="zhongbei"
                    />
                  </label>
                  <label className="join-field">
                    <span>组织类型</span>
                  ...
                  <label className="join-field">
                    ...
                    <input
                      value={org.parentRef}
                      onChange={(e) => updateOrg("parentRef", e.target.value)}
                      placeholder="v://cn.zhongbei"
                    />
                  </label>
                  <label className="join-field">
                    <span>公司名称</span>
                  ...
                  <label className="join-field">
                    ...
                    <input
                      type="date"
                      value={org.certValidUntil}
                      onChange={(e) => updateOrg("certValidUntil", e.target.value)}
                    />
                  </label>
                  <label className="join-field">
                    <span>注册资本（元）</span>
                  ...
                  <label className="join-field">
                    ...
                    <input
                      ...
                      onChange={(e) => updateOrg("establishedAt", e.target.value)}
                    />
                  </label>
                  <label className="join-field">
                    <span>地址</span>
                ...
                <div className="join-qual-head">
                  ...
                  <div className="join-actions">
                    ...
                    <button
                      ...
                      type="button"
                      onClick={() => setQualifications((prev) => [...prev, createQualFromPreset(qualPreset)])}
                    >
                      新增资质
                    </button>
                  </div>
                          ...
                          <div className="join-scope-options">
                            {preset.scopes.map((scope) => (
                              <label key={`${q.id}-${scope}`} className="join-scope-option">
                                <input
                                  type="checkbox"
                                  checked={(q.scope || []).includes(scope)}
                                  onChange={(e) => toggleScope(q.id, scope, e.target.checked)}
                                />
                                <span>{scope}</span>
                              </label>
                            ))}
            ...
            {step === 3 ? (
              <section className="panel join-card">
                ...
                <div className="join-mode-tabs">
                  <button
                    type="button"
                    className={`join-tab ${importMode === "file" ? "active" : ""}`}
                  ...
                  <button
                    type="button"
                    className={`join-tab ${importMode === "manual" ? "active" : ""}`}
                    ...
                    <div className="join-grid">
                      <article className="join-mini-preview">
                        ...
                        <input
                          type="file"
                          accept=".csv,.xlsx"
                          onChange={(e) => setEngineerFile(e.target.files?.[0] || null)}
                        />
                        <button
                          className="join-btn"
                          type="button"
                          onClick={uploadEngineerFile}
                          disabled={!engineerFile || busy}
                        >
                          上传工程师文件
                        </button>
                      </article>
                      <article className="join-mini-preview">
                        ...
                        <input
                          type="file"
                          accept=".csv,.xlsx"
                          onChange={(e) => setExecutorFile(e.target.files?.[0] || null)}
                        />
                        <button
                          className="join-btn"
                          type="button"
                          onClick={uploadExecutorFile}
                          disabled={!executorFile || busy}
                        >
                          上传执行体文件
                        </button>
                      </article>
                  ...
                  <div className="join-mode-content">
                    <div className="join-rows-actions">
                      <button
                        className="join-btn ghost"
                        type="button"
                        onClick={() => setManualRows((prev) => [...prev, createManualRow("engineer")])}
                      >
                        新增工程师行
                      </button>
                      <button
                        className="join-btn ghost"
                        type="button"
                        onClick={() => setManualRows((prev) => [...prev, createManualRow("executor")])}
                      >
                        新增执行体行
                      </button>
                      ...
                      {manualRows.map((row) => (
                        <div className="join-manual-row" key={row.id}>
                          <select
                            ...
                            onChange={(e) =>
                              setManualRows((prev) =>
                                prev.map((it) => (it.id === row.id ? { ...it, kind: e.target.value } : it)),
                              )
                            }
                          >
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setManualRows((prev) =>
                                prev.map((it) => (it.id === row.id ? { ...it, name: e.target.value } : it)),
                              )
                            }
                          />
                          <input
                            ...
                            onChange={(e) =>
                              setManualRows((prev) =>
                                prev.map((it) => (it.id === row.id ? { ...it, idCard: e.target.value } : it)),
                              )
                            }
                          />
                          {row.kind === "engineer" ? (
                            <select
                              value={row.registerType}
                              onChange={(e) =>
                                setManualRows((prev) =>
                                  prev.map((it) => (it.id === row.id ? { ...it, registerType: e.target.value } : it)),
                                )
                              }
                            >
                            ...
                            <select
                              ...
                              onChange={(e) =>
                                setManualRows((prev) =>
                                  prev.map((it) => (it.id === row.id ? { ...it, roleCode: e.target.value } : it)),
                                )
                              }
                            >
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setManualRows((prev) =>
                                prev.map((it) => (it.id === row.id ? { ...it, specialty: e.target.value } : it)),
                              )
                            }
                          />
                          <input
                            ...
                            onChange={(e) =>
                              setManualRows((prev) =>
                                prev.map((it) => (it.id === row.id ? { ...it, certNo: e.target.value } : it)),
                              )
                            }
                          />
                          <input
                            ...
                            onChange={(e) =>
                              setManualRows((prev) =>
                                prev.map((it) => (it.id === row.id ? { ...it, position: e.target.value } : it)),
                              )
                            }
                          />
                          <input
                            ...
                            onChange={(e) =>
                              setManualRows((prev) =>
                                prev.map((it) => (it.id === row.id ? { ...it, maxConcurrent: e.target.value } : it)),
                              )
                            }
                          />
                          <input
                            ...
                            onChange={(e) =>
                              setManualRows((prev) =>
                                prev.map((it) => (it.id === row.id ? { ...it, validUntil: e.target.value } : it)),
                              )
                            }
                          />
                          <button
                            className="join-link-btn danger"
                            type="button"
                            onClick={() => setManualRows((prev) => prev.filter((it) => it.id !== row.id))}
                          >
                            删除
                          </button>
                        </div>
              ...
              <section className="panel join-card">
                <div className="join-card-head">
                  ...
                  <div className="join-actions">
                    <a
                      ...
                      href={`/partner-profile?namespace=${namespaceCode}`}
                      target="_blank"
                      rel="noreferrer"
                    >
                      查看能力声明页
                ...
                <div className="join-result-grid">
                  ...
                  <article>
                    <p>激活资质 UTXO</p>
                    <strong>
                      {Array.isArray(registerResult?.qualification_genesis)
                        ? registerResult.qualification_genesis.length
                        : 0}
                    </strong>
                  </article>
                  <article>
                ...
                <div className="join-grid">
                  <article className="join-log-card">
                    ...
                    <ul className="join-ref-list">
                      {[
                        ...(Array.isArray(registerResult?.qualification_genesis)
                          ? registerResult.qualification_genesis
                          : []),
                ...
                <article className="join-resolver">
                  ...
                  <div className="join-grid">
                    <label className="join-field">
                      <span>spu_ref</span>
                      <input
                        value={resolverPayload.spu_ref}
                        onChange={(e) => setResolverPayload((prev) => ({ ...prev, spu_ref: e.target.value }))}
                      />
                    </label>
                    <label className="join-field">
                    ...
                    <label className="join-field">
                      <span>tenant_id</span>
                      <input
                        value={resolverPayload.tenant_id}
                        onChange={(e) => setResolverPayload((prev) => ({ ...prev, tenant_id: e.target.value }))}
                      />
                    </label>
                  </div>
                  <div className="join-actions">
                    <button className="join-btn" type="button" onClick={runResolver} disabled={busy}>
                      {busy ? "执行中..." : "执行 Resolver 校验"}
                    </button>
                  </div>
